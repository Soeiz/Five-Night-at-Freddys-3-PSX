...
int main(void) {
    ...
    {   
        ...
        if (linesactive == 1) {
            ...
            if (linesframes == 24) {
                LoadTexture(_binary_tim_titlemenu_lines_6_tim_start, &lines);
            }
            if (linesframes == 28) {
                linesactive = 0;
                linesframes = 0;
        ...
        if (menu == 0) { //Menu Screen
            if (helpwantedposter == 0) {
                ...
                CDreadOK = CdRead( (int)BtoS(filePos.size), (u_long *)dataBuffer, CdlModeSpeed );
                CDreadResult = CdReadSync(0, 0);
                LoadTexture(dataBuffer, &menuselections); 
                free(dataBuffer);
                loadFile = "\\LOADGAME.TIM;1";
                CdSearchFile( &filePos, loadFile);
                dataBuffer = malloc( BtoS(filePos.size) * CD_SECTOR_SIZE );
                CdControl(CdlSetloc, (u_char *)&filePos.pos, CtrlResult);
                // Read data and load it to dataBuffer
                ...
                CDreadOK = CdRead( (int)BtoS(filePos.size), (u_long *)dataBuffer, CdlModeSpeed );
                CDreadResult = CdReadSync(0, 0);
                LoadTexture(dataBuffer, &name); 
                free(dataBuffer);
                loadFile = "\\SLTBUT.TIM;1";
                CdSearchFile( &filePos, loadFile);
                dataBuffer = malloc( BtoS(filePos.size) * CD_SECTOR_SIZE );
                CdControl(CdlSetloc, (u_char *)&filePos.pos, CtrlResult);
                // Read data and load it to dataBuffer
        ...
        if (menu == 1) { //Loading/Starting night screen
            ...
            if (loadingframe > 360 && loadingframe < 615) {
...
void menuselectionfunc(void) { //LONG asf lmaoo
    ...
    if (menuselection == 1) {
        MovVectormenuselectionbutton.vy = 2;
    }
    if (menuselection == 2) {
       MovVectormenuselectionbutton.vy = 15;
    }
    if (menuselection == 3) {
       MovVectormenuselectionbutton.vy = 27;
    }
    if (menuselection == 4) {
       MovVectormenuselectionbutton.vy = 39;
    }
